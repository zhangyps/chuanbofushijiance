...
def predict(model,
    ...
    progbar_pred = progbar.Progbar(target=len(img_lists[0]), verbose=1)    #进度条
    color_map = visualize.get_color_map_list(256, custom_color=custom_color)
    with paddle.no_grad():
        for i, im_path in enumerate(img_lists[local_rank]):
            input = cv2.imread(im_path)             #####就是原图
            ...
            added_image = utils.visualize.visualize(
                input, im_path, pred, color_map, weight=0.6)
            added_image_path = os.path.join(added_saved_dir, im_file)
            mkdir(added_image_path)
            cv2.imwrite(added_image_path, added_image)

            # save pseudo color prediction
            pred_mask = utils.visualize.get_pseudo_color_map(im_path, pred, color_map)
